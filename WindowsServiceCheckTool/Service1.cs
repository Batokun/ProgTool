...
namespace WindowsServiceCheckTool
{
    public partial class Service1 : ServiceBase
    {
        ...
        void writeErrorLogT1(DateTime timeYomi)
        {
            ...
            if (ErrorLogStringT1.Length > 0)
            {
                ...
                else
                {
                    ...
                    if (lastMailSent.AddHours(1) < DateTime.Now)
                    {
                        if (!Directory.Exists(LogDir))
                            Directory.CreateDirectory(LogDir);
                        using (StreamWriter sw = System.IO.File.CreateText(LogDir + timeYomi.ToString("yyyyMMddHHmmss") + "T1-エラーログ.txt"))
        ...
        public void CheckInputFolderT1(DateTime yomi1)
        {
            try
            {
                ...
                if (InputFolderFileNamesT1.Count > 0)
                {
                    if (InputFolderFileNamesBkT1.Count > 0)
                    {
                        int cntold = 0;
                        ...
                        {
                            stop = true;
                            ErrorLogStringT1.AppendLine("エラーT1：" + DateTime.Now.ToString() + "：" + "PROGTOOLでエラーが発生したか、停止している恐れがあります。確認ください。</br>");
                        }
        ...
        public void CheckInputFolderT2(DateTime yomi2)
        {
            try
            {
                ...
                if (InputFolderFileNamesT2.Count > 0)
                {
                    if (InputFolderFileNamesBkT2.Count > 0)
                    {
                        int cntold = 0;
                        ...
                        {
                            stop = true;
                            ErrorLogStringT2.AppendLine("エラーT2：" + DateTime.Now.ToString() + "：" + "PROGTOOLでエラーが発生したか、停止している恐れがあります。確認ください。</br>");
                        }
        ...
        public void SendMail(StringBuilder ErrorLogCheckString)
        {
            using (MailMessage mail = new MailMessage())
            {
                ...
                mail.To.Add("easyweb-cloud-support@icraft.jp");
